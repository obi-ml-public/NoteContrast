# Python file to create the pre-training dataset. We read all encounter data
# and derive the ICD sequences for the patient.
...
class ICDDataset(object):
    ...
    def __get_grouped_sequence_df(self, sequence_df_merged):
        return (
            sequence_df_merged
            .groupby([self._patient_id_column, self._patient_encounter_id_column + '_x'])
            .agg(
            ...
            .rename(
                columns={
                    self._patient_encounter_id_column + '_x': self._patient_encounter_id_column,
                }
            )
        )
    ...
    def convert_parquet_to_json(sequence_df):
        for row in sequence_df.itertuples():
            ...
            yield {
                ...
                'patient_encounter_id': str(row.PatientEncounterID),
                'text': row.icd_code_sequence,
...
def run_transform():
    ...
    output_file = f'{Path(args.output_folder)}/{Path(input_file).stem}.json'

    encounters_df = pd.read_parquet(input_file)
    encounters_df = encounters_df[
    ...
    sequence_df = icd_dataset.get_sequence_df(encounters_df=encounters_df).reset_index()
    print('Shape of sequence dataframe: ', sequence_df.shape)
    sequence_df.drop_duplicates([patient_id_column, contact_date_column, icd_sequence_column], inplace=True)
    sequence_df.reset_index(drop=True, inplace=True)
    print('Shape of sequence dataframe after dropping duplicates: ', sequence_df.shape)
    ...
    print('Finished processing: ', str(Path(input_file).stem))
