...
from pre_train_datasets.text_transformations import MLMTransform, TextTransform

from model_helpers import ModelHelpers
from metrics import MLMMetrics
...
class MLMPreTrainer(object):
    ...
    def __init__(
            self,
            training_args,
            tokenizer: Optional[SpacyTokenizer],
    ...
    def set_trainer(
        ...
        # def preprocess_logits_for_metrics(logits, labels):
        #     logits_dict = {
        #         'clip_projection': logits[0],
        #         'hidden_state_cls': logits[1][-1][:, 0, :],
        #         'hidden_state': logits[1][-1][:, 1, :]
        #     }
        #     return logits_dict
...
def main():
    ...
    # Wan DB project name
    os.environ["WANDB_PROJECT"] = f"note_pretraining_mlm"
    ...
    model = model_helpers.get_mlm_model(
